...
status_t allocate_page(Process *process, addr_t address, addr_t physical_address) {
  ...
  // 检查物理内存是否足够
  if (!is_physical_memory_available(physical_address)) {
    return ERROR;  // 如果物理内存不足，返回错误
  }
  ...
  // 如果不存在，则分配L2页表
    ...
    if (process->page_table.entries[l1_index].entries == NULL) {
      return ERROR;  // 如果分配失败，返回错误
    }
  }

  // 设置L2页表项
  PTE *pte = &process->page_table.entries[l1_index].entries[l2_index];
  pte->frame = physical_address >> OFFSET_BITS;
  pte->valid = 1;

  // 更新物理内存状态
  main_memory->pages[pte->frame] = (Physical_page *)(uintptr_t)physical_address;
